...
import { gridToScreen, snapGrid, screenToGrid, resolveIsoConfig } from "../render/isoProjector.js";
...
export class BuildManager {
  ...
  _drawIsoDiamond(g, tx, ty, fillColor, fillAlpha, strokeColor, strokeAlpha) {
    const a = gridToScreen(tx, ty, this.infiniteCfg);
    const b = gridToScreen(tx + 1, ty, this.infiniteCfg);
    const c = gridToScreen(tx + 1, ty + 1, this.infiniteCfg);
    const d = gridToScreen(tx, ty + 1, this.infiniteCfg);
    g.clear();
